    ...
    <section class="bg-gray-50 dark:bg-gray-900 p-3">
        <div class="mx-auto max-w-screen-3xl px-4 lg:px-12">
            <h2 class="mb-4 py-2 text-3xl font-bold text-gray-900 dark:text-white">{{ competition.title }}, <br
                class="sm:hidden block"/>{{ dayjs(competition.start_date).format("DD.MM.YYYY") }}<template v-if="!dayjs(competition.start_date).isSame(dayjs(competition.end_date))"> - {{ dayjs(competition.end_date).format('DD.MM.YYYY') }}</template>
                <span v-if="isSecretary"> ({{ trans("participants.createdBy") }}: {{ competition.created_by}})</span></h2>
            <!--группа-->
                            ...
                            <template
                                ...
                                <tr class="border-t dark:border-gray-700">
                                    ...
                                    <td class="px-4 py-3 font-medium text-gray-900 dark:text-white whitespace-normal text-ellipsis overflow-hidden">
                                        {{ competition.ui_language !== "ru" ? tr(participant.athlete.surname) : participant.athlete.surname}} {{
                                            competition.ui_language !== "ru" ? tr(participant.athlete.first_name) : participant.athlete.first_name
                                        }}{{
                                            participant.athlete.patronymic !== null ? " " + (competition.ui_language !== "ru" ? tr(participant.athlete.patronymic) : participant.athlete.patronymic) : ""
                                        }}
                                    </td>
