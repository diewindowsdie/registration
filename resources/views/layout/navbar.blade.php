...
<ul class="space-y-2 mt-0" data-accordion="open" data-active-classes="text-black dark:text-white"
    ...
    @foreach($competitions as $competition_item)
    <li>
        <button type="button"
                @if(($competition->id ?? null) === $competition_item->id)
                    class="flex items-center p-2 w-full text-base font-normal rounded-lg transition duration-75 group text-black dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700"
                    aria-expanded="true"
                @else
                    class="flex items-center p-2 w-full text-base font-normal rounded-lg transition duration-75 group text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700"
                    aria-expanded="false"
                @endif
                @if(\App\Auth\ClientCertificateOrBasicAuthAuthenticator::isAuthenticated() || $competition_item->isRegistrationOpen())
                aria-controls="dropdown-competitions-{{ $competition_item->id }}"
                data-accordion-target="#dropdown-competitions-{{ $competition_item->id }}"
            @endif
        >
            <span
        ...
        @if(\App\Auth\ClientCertificateOrBasicAuthAuthenticator::isAuthenticated() ||
        $competition_item->isRegistrationOpen())
        <div class="grid grid-cols-8 pl-6 hidden" id="dropdown-competitions-{{ $competition_item->id }}">
            @if($competition_item->isRegistrationOpen())
